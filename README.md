...
was specified). By default this macro generates a mesh grid with double the
density of the default grid in both the X and Y dimensions (i.e. if `bed_mesh`
specifies `probe_count: 5,5` then this macro will generate a 9x9 grid, which
doubles the probe_count while preserving all of the points on the default grid).
...
* `variable_start_try_saved_surface_mesh` *(default: True)* If enabled and
